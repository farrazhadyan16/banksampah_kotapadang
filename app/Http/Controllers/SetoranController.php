...
class SetoranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $nasabahId =
            $user->role === "nasabah" ? $user->id : $request->id_nasabah;

        if (!$nasabahId) {
            return back()->withErrors([
                "id_nasabah" => "Pilih nasabah terlebih dahulu.",
            ]);
        }
    ...
    public function konfirmasiSetoran($id)
    {
        ...
        if ($setoran->status !== "Completed") {
            User::where("id", $setoran->id_nasabah)->increment(
                "saldo",
                $setoran->total_harga
            );

            foreach ($setoran->details as $detail) {
                Sampah::where("id", $detail->id_sampah)->increment(
                    "jumlah",
                    $detail->jumlah_sampah
                );
            }
