        ...
        <form action="{{ route('setoran.konfirmasi') }}" method="POST">
            ...
            <div class="row g-3">
                ...
                @foreach ($items as $key => $label)
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" 
                            value="{{ $data['jumlah_' . $key] ?? 0 }}" readonly>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Harga {{ $label }}</label>
                        <input type="text" class="form-control" 
                            value="Rp. {{ number_format($data['harga_' . $key] ?? 0, 0, ',', '.') }}" readonly>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Harga Total {{ $label }}</label>
                        <input type="text" class="form-control" 
                            value="Rp. {{ number_format(($data['jumlah_' . $key] ?? 0) * ($data['harga_' . $key] ?? 0), 0, ',', '.') }}" readonly>
                    </div>
                @endforeach
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" class="form-control text-success fw-bold fs-5" 
